...
import { ChatMessages, ChatMessageDocument } from './entities/chat_message.entity';
...
export class ChatMessagesService {
  ...
  async findAllByChatHistoryId(chatHistoryId: string): Promise<ChatMessages[]> {
    return this.chatMessageModel.find({ chatId: new Types.ObjectId(chatHistoryId) }).exec();
  }

  async createMessage(dto: CreateChatMessageDto): Promise<ChatMessages> {
